...
const encryptDataUsingEncryptionKey = (data, encryptionKey) => {
  const cryptr = new Cryptr(encryptionKey);
  return cryptr.encrypt(data);
};

const decryptDataUsingEncryptionKey = (data, encryptionKey) => {
  const cryptr = new Cryptr(encryptionKey);
  return cryptr.decrypt(data);
};
...
const decryptUsingSignatures = async (data, signature) => {
  let {publicKey, privateKey } = getKeyPair(signature);
